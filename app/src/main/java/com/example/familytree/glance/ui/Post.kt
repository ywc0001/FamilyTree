...
package com.example.familytree.glance.ui
...
import com.example.familytree.glance.ui.theme.FamilytreeGlanceTextStyles
import com.example.familytree.model.Post
import com.example.familytree.ui.MainActivity
...
private fun openPostDetails(context: Context, post: Post): Action {
    ...
    return actionStartActivity(
        Intent(
            Intent.ACTION_VIEW,
            "$FAMILYTREE_APP_URI/home?postId=${post.id}".toUri(),
            context,
...
fun PostDescription(title: String, metadata: String, modifier: GlanceModifier) {
    Column(modifier = modifier) {
        Text(
            text = title,
            maxLines = 3,
            style = FamilytreeGlanceTextStyles.bodyLarge
                .copy(color = GlanceTheme.colors.onBackground)
        )
        Spacer(modifier = GlanceModifier.height(4.dp))
        Text(
            text = metadata,
            style = FamilytreeGlanceTextStyles.bodySmall
                .copy(color = GlanceTheme.colors.onBackground)
        )
    }
}
